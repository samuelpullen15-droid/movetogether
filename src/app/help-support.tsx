      ...
      if (user?.id) {
        ...
        try {
          ...
          if (refreshError || !accessToken) {
            console.warn('[HelpSupport] Session refresh failed, continuing without identity verification');
          } else {
            // Call the Edge Function to get Intercom JWT
            const supabaseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL;
            const supabaseAnonKey = process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY;

            const response = await fetch(`${supabaseUrl}/functions/v1/get-intercom-token`, {
              method: 'POST',
              headers: {
          ...
          console.warn('[HelpSupport] Identity verification error, continuing without:', tokenErr);
        }
      ...
      >
        ...
        >
          ...
          >
            <View className="p-5">
              <View className="flex-row items-center">
                ...
                <ExternalLink size={20} color={colors.isDark ? '#4a4a4a' : '#9ca3af'} />
              </View>
            </View>
          </Pressable>
          ...
          <Pressable
            ...
            style={{ backgroundColor: colors.card, opacity: isLoadingChat ? 0.7 : 1 }}
          >
            <View className="p-5">
              <View className="flex-row items-center">
                <View
                  className="w-12 h-12 rounded-full items-center justify-center"
                  style={{ backgroundColor: colors.isDark ? 'rgba(139, 92, 246, 0.2)' : 'rgba(139, 92, 246, 0.1)' }}
                >
                  {isLoadingChat ? (
                ...
                <View className="flex-1 ml-4">
                  ...
                  <Text style={{ color: colors.textSecondary }} className="text-sm mt-1">
                    {isLoadingChat ? 'Connecting...' : 'Get help from our support team'}
                  </Text>
                </View>
              </View>
            </View>
